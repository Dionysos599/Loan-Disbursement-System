...
# Test backend services (runtime)
echo -e "\n${BLUE}Testing Backend Services (Runtime):${NC}"
...
# Test backend compilation (optional - only if services are not running)
...
if cd backend/data-ingestion && mvn clean compile -q > /dev/null 2>&1; then
    ...
    echo -e "${YELLOW}⚠ SKIP (service running)${NC}"
    cd - > /dev/null
fi
